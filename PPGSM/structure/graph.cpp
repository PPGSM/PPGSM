      ...
      for (auto p : result_paths) {
        ...
        for (auto e : p) {
          ...
          if (N.node->user == false) {
            ...
            //Rst.parms_id();
            //     	        			eval.mod_switch_to_inplace(risk,
            //     last_parms_id);
        ...
        clock_t end_ = clock();
        cout << "calculation time for one path: "
...
struct degree getDegree(struct Graph &G) {
  ...
  for (s = G.node->begin(); s != G.node->end(); ++s) {
    ...
    for (Nx = (*s).node->Neighbors->begin(); Nx != (*s).node->Neighbors->end();
         ++Nx) {
...
void PrAtkSuccess(struct Graph &G, int startNumber, int destNumber,
  ...
  for (auto inode : *(G.node)) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      Ciphertext cos_eff1, cos_eff2, cos_eff3, cos_eff4, cos_x2, cos_x4, cos_x6,
          cos_x6_temp;
      ...
      */
      ...
      Plaintext Temp0, Temp1, Temp2, Temp3, Temp4, Temp5, Temp6, Temp7, Temp8,
          Temp9, Temp10;
      ...
      Ciphertext temp0, temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8,
          temp9, temp10, temp, temp_a, temp_b;
      ...
      */
      ...
      Plaintext secondTemp1, secondTemp2, secondTemp3, secondTemp4, secondTemp5,
          secondTemp6, secondTemp7, secondTemp8, secondTemp9, secondTemp10;
      ...
      Ciphertext secondtemp1, secondtemp2, secondtemp3, secondtemp4,
          secondtemp5, secondtemp6, secondtemp7, secondtemp8, secondtemp9,
          secondtemp10, secondtemp, secondtemp_a, secondtemp_b;
...
void levelUpSEAL(Ciphertext &pr, seal::Evaluator &eval,
                 seal::PublicKey public_key, seal::RelinKeys &relin_keys,
...
void cumulPrAtkSuccessProbe(struct Graph &G, struct nodeList &N,
                            vector<int> &inDegree, queue<int> &searchList,
                            vector<Ciphertext> &tempNodePr,
                            vector<Ciphertext> &finalNodePr, int source,
                            seal::Evaluator &eval, seal::PublicKey public_key,
  ...
  while (searchList.empty() != true) {
    ...
    if (calFin[current] == 0 && inDegree[current] == 0) {
      ...
      if (current != source) {
        ...
        levelUpSEAL(finalNodePr[current], eval, public_key, relin_keys, context,
                    secret_key);
      } else {
      ...
      for (iter = currentNode.node->Neighbors->begin();
           iter != currentNode.node->Neighbors->end(); iter++) {
        ...
        levelUpSEAL(tempNodePr[next], eval, public_key, relin_keys, context,
                    secret_key);
      }
    }
  }
...
void cumulPrAtkSuccess(struct Graph &G, int startNumber, seal::Evaluator &eval,
  ...
  for (auto inode : *(G.node)) {
    ...
    cumulPrAtkSuccessProbe(G, inode, inDegree, searchList, tempNodePr,
                           finalNodePr, startNumber, eval, public_key,
                           relin_keys, context, secret_key);
    /*
            //test result
            for(int i=0;i<graphSize;i++){
                    Decryptor decryptor(context, secret_key);
