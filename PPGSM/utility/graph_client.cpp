...
void createDummyNode(struct Graph &G, int weight, int impact, double pr,
  ...
  createNode(G, G.node->size(), weight, impact, pr, logPr, patch, inv_patch,
             false, false, public_key, context);
}
  ...
  while (c > 0) {
    ...
    for (int i = 0; i < TrSize; i++) {
      if (strncmp(t2, Tr[i].type, len) == 0) {
        /////test for spearman
        if (i == 3) {
          createNode(G, nodeNumber, Tr[i].cost, Tr[i].impact, Tr[i].pr,
                     Tr[i].logPr, Tr[i].patch, Tr[i].inverse_patch, false,
                     false, public_key, context);
        } else
          createNode(G, nodeNumber, Tr[i].cost, Tr[i].impact, Tr[i].pr,
                     Tr[i].logPr, Tr[i].patch, Tr[i].inverse_patch, false, true,
                     public_key, context);
        nodeNumber++;
        User = false;
        break;
    ...
    if (User == true) {
      createNode(G, nodeNumber, 0, 0, 0.0, -100000, 1, 1, true, true,
                 public_key, context);
      nodeNumber++;
    }
    c = read(nod, &T, 1);
...
void queryData(std::vector<std::string> &OSlist, std::string &targetOS, int NID,
  ...
  for (int i = 0; i < Tablesize; i++) {
    if (targetOS.compare(OSlist[i]) == 0)
      break;
    index++;
  }
  if (index >= Tablesize)
  ...
  std::ofstream ctresult("query/queryProduct" + to_string(NID),
                         std::ofstream::binary);
...
void createTopology(Graph &G) {
  ...
  for (auto N : *(G.node)) {
    ...
    ofstream nodeTruth("GSMConstruction/nodes/nodeTruth" + to_string(NID),
                       std::ofstream::binary);
    ...
    for (auto Ne : *(Nn->Neighbors)) {
      ofstream edgeTruth("GSMConstruction/edges/edgeTruth" +
                             to_string(edge_counter),
                         std::ofstream::binary);
